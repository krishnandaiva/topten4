...
function adv_nav_price_input_listener(evt){
    ...
    var numFrom = adv_nav_round($('adv-nav-price-from---' + sKey).value),
        numTo   = adv_nav_round($('adv-nav-price-to---' + sKey).value);
...
//    adv_nav_add_params('price', numFrom + ',' + numTo, true);
    adv_nav_add_params(sKey, numFrom + ',' + numTo, true);
    adv_nav_make_request();         
}
...
//    var items = $('adv-nav-container').select('.pages a', '.view-by a');
    var items = $('adv-nav-container').select('.pages a', '.view-mode a', '.sort-by a');
    var i, n = items.length;
    for (i=0; i<n; ++i){
...
// start new fix code    
    ...
    for (i=0; i<n; ++i)
    {
        btn = $('adv-nav-price-go---' + items[i].value);
        if (Object.isElement(btn)){
            Event.observe(btn, 'click', adv_nav_price_input_listener);
            Event.observe($('adv-nav-price-from---' + items[i].value), 'keypress', adv_nav_price_input_listener);
            Event.observe($('adv-nav-price-to---' + items[i].value), 'keypress', adv_nav_price_input_listener);
        }
    }
// finish new fix code    
}
  
function adv_nav_create_slider(width, from, to, min_price, max_price, sKey) 
{
    var price_slider = $('adv-nav-price-slider' + sKey);
...
//            t = adv_nav_round(max_price*values[1]/width);
        var f = adv_nav_calculate(width, from, to, min_price, max_price, values[0]),
            t = adv_nav_calculate(width, from, to, min_price, max_price, values[1]);
           
//        adv_nav_add_params('price', f + ',' + t, true);
        adv_nav_add_params(sKey, f + ',' + t, true);
        
        // we can change values without sliding  
        $('adv-nav-range-from' + sKey).update(f); 
        $('adv-nav-range-to' + sKey).update(t);
            
        adv_nav_make_request();  
      },
      onSlide: function(values) { 
//          $('adv-nav-range-from' + sKey).update(adv_nav_round(max_price*values[0]/width));
//          $('adv-nav-range-to' + sKey).update(adv_nav_round(max_price*values[1]/width));
          $('adv-nav-range-from' + sKey).update(adv_nav_calculate(width, from, to, min_price, max_price, values[0]));
          $('adv-nav-range-to' + sKey).update(adv_nav_calculate(width, from, to, min_price, max_price, values[1]));
      }
    });
}
